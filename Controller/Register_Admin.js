...
exports.Register_Admin = (req, res) => {
  ...
  const { username, contrasena, rol_id } = req.body;

  if (!rol_id) {
    return res.status(400).json({ message: 'Rol no proporcionado' });
  }

  const sql = 'INSERT INTO usuarios (nombre_usuario, contrasena, rol_id) VALUES (?, ?, ?)';
  db.query(sql, [username, contrasena, rol_id], (err, result) => {
    if (err) {
